...
export class CommentStorage {
  ...
  getAllComments(): Comment[] {
    const stored = this.context.globalState.get<Comment[]>(
      CommentStorage.STORAGE_KEY
    );
    return stored || [];
  }
  ...
  getCommentStats(): {
    ...
    return {
      ...
      averageCommentsPerFile:
        Math.round((comments.length / uniqueFiles.size) * 100) / 100,
